...
const PlanList = ({ plans }) => {
  ...
  return (
    <div className="flex flex-col lg:flex-row items-center justify-center max-w-[1280px] mx-auto gap-y-4">
      {plans.map((plan, curentIndex) => {
        const { name, price, list, delay } = plan;
        return (
          <div
            onClick={() => setIndex(curentIndex)}
            key={curentIndex}
            className="w-full md:max-w-[620px] lg:max-w-[405px] rounded-sm px-4 lg:min-h-[550px]"
            data-aos = "fade-up" data-aos-offset ="200" data-aos-delay={delay}
          >
            <div
              className={`${
                curentIndex === index
            ...
            >
              <div className="flex flex-row lg:flex-col gap-x-8 gap-y-8 lg:gap-x-0 items-center">
                ...
                <div>
                  ...
                  <div className="text-[40px] lg:text-[50px] font-primary font-extrabold text-center flex flex-col justify-center items-center">
